...
var noMoreMsg = '<p class="nomore">未找到搜索结果。<br>请尝试不同搜索方式。</p>';
...
function footSort() {
	...
	for (var i = 0; i < footList.length ; i++ ) {
		...
		var mapLayer = '<div class="'+footList[i].fCate+' '+footList[i].fCnty+' '+ footList[i].fIden +'">';
			...
			mapLayer +='<tbody>';
			mapLayer +='<tr>';
			mapLayer +='<th scope="row">公司名称</th>';
			mapLayer +='<td class="foot_company">'+footList[i].fComp+'</td>';
			mapLayer +='</tr>';
			mapLayer +='<tr>';
			mapLayer +='<th scope="row">地址</th>';
			...
			if (footList[i].fUrl==''){
				...
				mapLayer +='<td class="foot_email"><a href="http://'+footList[i].fUrl+'" target="_blank"">'+footList[i].fUrl+'</a></td>';
			}
